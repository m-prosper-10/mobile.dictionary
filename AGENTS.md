...
- **Android**: drawer + stack, Lucide via `@/components/icon`; test with **Expo Go** (`npm start` → scan QR) or dev build (`npm run android`)
- **Web**: sidebar layout in `_layout.web.tsx`, Radix menus/tooltips, responsive `md:` breakpoints (`npm run web` or `npx agent-browser`)
...
```bash
npm install
npm start                # Dev server — scan QR with Expo Go on Android
```

| Target | Command | When |
|--------|---------|------|
| Android (Expo Go) | `npm start` → Expo Go app | Default for UI, navigation, styling |
| Android (dev build) | `npm run android` | Native module not in Expo Go, pre-release QA |
| Web | `npm run web` or `npx agent-browser` | Sidebar, Radix, responsive layout |
...
iOS instructions only matter when you are explicitly targeting Apple platforms on a Mac.
